...
{
    internal class CovidDose2Downloader
    {
        private static int documentTypeId = 29544;
        private string baseUrl;
        private string userName;
        private string password;
        private string connectionString;
        public CovidDose2Downloader(string baseUrl, string userName, string password, string connectionString)
        {
        ...
        public async Task Execute()
        {
            ...
            {
                var result = await connection.QueryAsync<CovidDose2Doc>(query.Sql, parameters);
                ...
                foreach (var document in documents)
                {
                    ...
                    if (File.Exists($"CovidDose2_docs/{document.DocumentId}.pdf"))
                    {
                    ...
                    try
                    {
                        ...
                        if (docResult.IsSuccessStatusCode)
                        {
                            ...
                            Directory.CreateDirectory("CovidDose2_docs");
                            using var fs = new FileStream($"CovidDose2_docs/{document.DocumentId}.pdf", FileMode.Create, FileAccess.Write, FileShare.None);
            ...
            File.WriteAllText("CovidDose2_docs.csv", csv.ToString());
        }
