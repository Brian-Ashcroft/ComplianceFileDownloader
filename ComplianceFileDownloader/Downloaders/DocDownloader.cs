...
{
    internal class DocDownloader
	{
		private readonly BlobStorageConfig _blobConfig;
		private readonly DocDownloadConfig _downloadConfig;
        public DocDownloader(BlobStorageConfig blobConfig, DocDownloadConfig downloadConfig)
        {
        ...
        public async Task Execute()
        {
            ...
            {
                new queryDto(_downloadConfig.PassedDocsToDownload,
                ...
                new queryDto(_downloadConfig.CLFailedDocsToDownload,
                ...
                new queryDto(_downloadConfig.QMFailedDocsToDownload,
                    @"SELECT DISTINCT TOP 500
            ...
            var parameters = new { DocumentTypeId = _downloadConfig.DocumentTypeId };
            foreach (var query in queries)
            {
                ...
                foreach (var document in documents)
                {
                    ...
                    if (File.Exists($"docs/{_downloadConfig.DocumentTypeName}_docs/{document.DocumentId}.pdf"))
                    {
                        continue;
                    }
                    try
                    {
                        ...
                        if (docResult.IsSuccessStatusCode)
                        {
                            ...
                            Directory.CreateDirectory($"docs/{_downloadConfig.DocumentTypeName}_docs");
                            using var fs = new FileStream($"docs/{_downloadConfig.DocumentTypeName}_docs/{document.DocumentId}.pdf", FileMode.Create, FileAccess.Write, FileShare.None);
            ...
            File.WriteAllText($"docs/{_downloadConfig.DocumentTypeName}_docs.csv", csv.ToString());
        }
