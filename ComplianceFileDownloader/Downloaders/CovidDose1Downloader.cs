...
{
    internal class CovidDose1Downloader
    {
        private static int documentTypeId = 28791;
        private string baseUrl;
        private string userName;
        private string password;
        private string connectionString;
        public CovidDose1Downloader(string baseUrl, string userName, string password, string connectionString)
        {
        ...
        public async Task Execute()
        {
            ...
            {
                var result = await connection.QueryAsync<CovidDose1Doc>(query.Sql, parameters);
                ...
                foreach (var document in documents)
                {
                    ...
                    if (File.Exists($"CovidDose1_docs/{document.DocumentId}.pdf"))
                    {
                    ...
                    try
                    {
                        ...
                        if (docResult.IsSuccessStatusCode)
                        {
                            ...
                            Directory.CreateDirectory("CovidDose1_docs");
                            using var fs = new FileStream($"CovidDose1_docs/{document.DocumentId}.pdf", FileMode.Create, FileAccess.Write, FileShare.None);
            ...
            File.WriteAllText("CovidDose1_docs.csv", csv.ToString());
        }
