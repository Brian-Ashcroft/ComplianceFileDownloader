...
{
    internal class TdapDownloader
    {
        private static int documentTypeId = 65;
        private string baseUrl;
        private string userName;
        private string password;
        private string connectionString;
        public TdapDownloader(string baseUrl, string userName, string password, string connectionString)
        {
        ...
        public async Task Execute()
        {
            ...
            {
                var result = await connection.QueryAsync<TdapDoc>(query.Sql, parameters);
                ...
                foreach (var document in documents)
                {
                    ...
                    if (File.Exists($"Tdap_docs/{document.DocumentId}.pdf"))
                    {
                    ...
                    try
                    {
                        ...
                        if (docResult.IsSuccessStatusCode)
                        {
                            ...
                            Directory.CreateDirectory("Tdap_docs");
                            using var fs = new FileStream($"Tdap_docs/{document.DocumentId}.pdf", FileMode.Create, FileAccess.Write, FileShare.None);
            ...
            File.WriteAllText("Tdap_docs.csv", csv.ToString());
        }
