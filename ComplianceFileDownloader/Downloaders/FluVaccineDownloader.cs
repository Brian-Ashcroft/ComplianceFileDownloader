...
{
    internal class FluVaccineDownloader
    {
        private static int documentTypeId = 766;
        private string baseUrl;
        private string userName;
        private string password;
        private string connectionString;
        public FluVaccineDownloader(string baseUrl, string userName, string password, string connectionString)
        {
        ...
        public async Task Execute()
        {
            ...
            {
                var result = await connection.QueryAsync<FluVaccineDoc>(query.Sql, parameters);
                ...
                foreach (var document in documents)
                {
                    ...
                    if (File.Exists($"FluVaccine_docs/{document.DocumentId}.pdf"))
                    {
                    ...
                    try
                    {
                        ...
                        if (docResult.IsSuccessStatusCode)
                        {
                            ...
                            Directory.CreateDirectory("FluVaccine_docs");
                            using var fs = new FileStream($"FluVaccine_docs/{document.DocumentId}.pdf", FileMode.Create, FileAccess.Write, FileShare.None);
            ...
            File.WriteAllText("FluVaccine_docs.csv", csv.ToString());
        }
